    ...
    if (chats.length > 0) {
        return chats.map(chat => {
                ...
                if (chat.messages.length > 0) {
                    ...
                    return (
                            <span onClick={() => setActiveChat(chat.id, recipientId)} key={chat.id}
                                  className={`chat__opponent w-100 d-flex align-items-center ${activeChat == chat.id && 'active'} ${messagesCount > 0 && 'has-messages'}`}>
                            {avatar == null
